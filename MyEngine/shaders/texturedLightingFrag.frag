...
vec3 calculatePointLight(light point_light)
{
	vec3 lightDir = normalize(point_light.pos - fs_in.fragPos);
	vec3 normal = normalize(fs_in.outNormals);
	vec3 viewPos = normalize(camPos - fs_in.fragPos);
	vec3 halfWayDir = normalize(lightDir + viewPos);			//used for blinn-phong lighting
	
	vec3 finalVec;
	
	normal = texture(mat.normalMap, fs_in.outTexCoords).rgb;
	...
	vec3 diffuseVec = diffuseFactor * mat.ambient * vec3(texture(mat.diffuseMap,fs_in.outTexCoords)) * mat.diffuse;

	//vec3 reflected = reflect(-lightDir, normal);
	float specularFactor = pow(max(dot(normal, halfWayDir),0.0),mat.shininess);

	vec3 specularVec = specularFactor * vec3(texture(mat.specularMap,fs_in.outTexCoords)) * mat.specular; 
...
vec3 calculateFlashLight(light flashLight)
{
	...
	vec3 lightDir = normalize(fs_in.fragPos - flashLight.pos);
...
vec3 calculateDirLightWithMaterial(light dir_light)
{
	...
	float shadowFactor = calculateShadowFactor(fs_in.fragPosLightSpace);
...
void main()
{
	vec3 dirLightVec = calculateDirLightWithMaterial(dirLight);
